...
func main() {
	...
	for _, districtName := range district {
		totalSoldPage := spider.GetSoldPageSpider(esClient, districtName)
		for page := 100; page <= totalSoldPage; page++ {
			wgSold.Add(1)
			time.Sleep(time.Duration(page*20) * time.Millisecond)
			go func(page int) {
				// fmt.Println("start spider", page)
				defer wgSold.Done()
				spider.GetSoldInfoSpider(esClient, districtName, page)
			}(page)
		}
	}
	wgSold.Wait()
